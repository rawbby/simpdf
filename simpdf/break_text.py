...
def break_text(text: str | None, line_width: float, style: TextStyle) -> list[Text]:
    """
    Breaks a single string of text into multiple TextLine objects that fit within the specified width.
    """
    return [Text(line, style=style) for line in _text_breaker(text, line_width, style)]
...
def break_block_text(text: str | None, line_width: float, style: TextStyle,
                     ...
                     max_horizontal_scale: float = 102.0) -> list[Text]:
    """
    ...
    result = []
    for line in lines[:-1]:
        result.append(Text(content_left=line, style=stretch_text(
    ...
    result.append(Text(content_left=lines[-1], style=stretch_text(
        lines[-1], target_width, style,
        min_word_space_factor=min_word_space_factor,
        min_char_space_factor=min_char_space_factor,
        min_horizontal_scale=min_horizontal_scale,
